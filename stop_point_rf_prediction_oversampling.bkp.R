...
for (i in 1:10) trn_data <- rbind(trn_data, nomad) ## each nomad replicated 10 times

## oversample "integration"
integration <- trn_data.raw[trn_data.raw[,16] == "integration",]
for (i in 1:20) trn_data <- rbind(trn_data, integration) ## each integration replicated 20 times

## oversample "ipf"
ipf <- trn_data.raw[trn_data.raw[,16] == "ipf",]
for (i in 1:20) trn_data <- rbind(trn_data, ipf) ## each ipf replicated 20 times

## oversample "sales"
sales <- trn_data.raw[trn_data.raw[,16] == "sales",]
for (i in 1:25) trn_data <- rbind(trn_data, sales) ## each sales replicated 25 times

## oversample "warehouse"
warehouse <- trn_data.raw[trn_data.raw[,16] == "warehouse",]
for (i in 1:30) trn_data <- rbind(trn_data, warehouse) ## each warehouse replicated 30 times
## table(trn_data.raw[,16])
## table(trn_data[,16])
## read test data
...
x <- t_subset_trn
x$new_reason <- NULL
y <- t_subset_trn$new_reason

rf.cv <- rfcv(x, y, cv.fold = 10)

with(rf.cv, plot(n.var, error.cv))
rf.cv$n.var
head(rf.cv$predicted)
## randomForest
library(randomForest)
library(caret)
###### commented for testing #######
## rfm1 <- randomForest(new_reason ~ ., subset_trn, ntree=1000)
rfm1 <- randomForest(new_reason ~ integ_cost_per_unit+tot_integ_cost+tot_time+tot_resale+integ_cost_per_hr
                     , t_subset_trn, ntree=1000, nodesize = 2, maxnodes = 36, replace = T, mtry=4)
...
rfm2 <- randomForest(new_reason ~ integ_cost_per_unit+tot_integ_cost+tot_time+tot_resale+integ_cost_per_hr
                     , t_subset_trn, ntree=1000, importance=T, nodesize = 2, maxnodes = 36, replace = T, mtry=4)
###### end of commented for testing #######
dev.new(width=10, height=8)
...
(overall_acc <- (15+1+1+8645+0+0)/9400) ## 0.9214894

## accuracy with of other classes except 'none'
(other_acc <- (1+15+1)/(153+64+289+74+24)) ## 0.0281457
